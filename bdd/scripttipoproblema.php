...
function buscarRegistro($search)
{
    global $mysqli;
    if (!empty($search)) {
        $search_query = "SELECT nombre FROM Tipo_reporte WHERE nombre='$search' LIMIT 1";
...
{
    global $mysqli;
    $select_query = "SELECT * FROM Tipo_reporte ORDER BY nombre ASC";
...
function add($nombre, $prioridad)
{
    ...
    $add_query = "INSERT INTO Tipo_reporte (nombre,prioridad)
...
function verRegistroMod($id)
{
    global $mysqli;
    if (!empty($id)) {
        $search_query = "SELECT * FROM Tipo_reporte WHERE idTipo='$id' LIMIT 1;";
        $result = $mysqli->query($search_query);
        if (!$result) {
            die('Error de consulta ');
        }
...
function mod($id, $nombre, $prioridad)
{
    ...
    $sql_mod = "UPDATE Tipo_reporte SET nombre='$nombre', prioridad='$prioridad' WHERE idTipo='$id' LIMIT 1";
    // Se llama la variable de conexión y se ejecuta el query
...
function del($id_del)
{
    ...
    $sql_del = "DELETE FROM Tipo_reporte WHERE idTipo = '$id_del' LIMIT 1";
    // Se llama la variable de conexión y se ejecuta el query
